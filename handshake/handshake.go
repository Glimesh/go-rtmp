...
// From: https://github.com/q191201771/lal/blob/0f213349645ede35ace26a18f170f60ca9e3c721/pkg/rtmp/handshake.go
...
func (c *HandshakeClientSimple) WriteC0C1(writer io.Writer) error {
	...
	binary.BigEndian.PutUint32(c.buf[5:9], 0) // 4字节模式串保持为0，标识是简单模式
	random1528(c.buf[9:])
...
func parseChallenge(b []byte, peerKey []byte, key []byte) []byte {
	...
	if ver == 0 {
		log.Println("handshake simple mode.")
		return nil
	}
	...
	if offs == -1 {
		log.Println("get digest offs failed. roll back to try simple handshake.")
		return nil
	}
	log.Println("handshake complex mode.")
...
func init() {
	...
	hack := []byte(fmt.Sprintf("random buf of rtmp handshake gen by %s", "some stuff"))
